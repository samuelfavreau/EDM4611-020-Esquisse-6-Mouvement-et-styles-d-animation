class Square_middle {
  ...
  //Constructor
  Square_middle(float speed, Easing animType) {
  ...
  void display() {
    ...
    sourceImage.rotate(radians(360 * s1));
    sourceImage.fill(hue, sat[satIndex % 3], 100);
    sourceImage.rect(0, 0, animW*s1, animH*s1);
    sourceImage.pop();

    sourceImage.push();
    sourceImage.translate(animW/2, animH/2);
    sourceImage.rotate(radians(360 * s2));
    sourceImage.fill(hue, sat[(satIndex + 1) % 3], 100);
    sourceImage.rect(0, 0, animW*s2, animH*s2);
    sourceImage.pop();

    sourceImage.push();
    sourceImage.translate(animW/2, animH/2);
    sourceImage.rotate(radians(360 * s3));
    sourceImage.fill(hue, sat[(satIndex + 2) % 3], 100);
    sourceImage.rect(0, 0, animW*s3, animH*s3);
    sourceImage.pop();
